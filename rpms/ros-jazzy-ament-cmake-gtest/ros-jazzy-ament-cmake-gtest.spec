...
Version:        2.5.6
Release:        1%{?dist}
Summary:        The ability to add GTest-based tests in the ament buildsystem in CMake

License:        Apache-2.0
URL:            https://github.com/ament/ament_cmake
# Upstream ships all ament_cmake_* subpackages in one monorepo tarball; this
# spec carves out ament_cmake_gtest with colcon's --packages-select.
...
# buildtool_export_depend -> Requires (needed by downstream builds): the
# ament_cmake_gtest-extras.cmake soft-finds gtest_vendor, and ament_add_gtest
# pulls in the ament_cmake_test harness. google-test system pkg is a soft
# fallback (vendor preferred), so it is not required.
Requires:       ros-jazzy-ament-cmake-test
Requires:       ros-jazzy-gtest-vendor

%description
The ability to add GTest-based tests in the ament buildsystem in CMake. Part of
the ament buildsystem for ROS 2 Jazzy, installed under %{_libdir}/ros-jazzy.
