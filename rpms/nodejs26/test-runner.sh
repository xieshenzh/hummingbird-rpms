...
while IFS= read -r test_line; do
  ...
  # Parse RLIMIT_AS directive from test file (upstream test runner convention)
  ULIMIT_PREFIX=""
  RLIMIT_AS=$(grep -oP '// RLIMIT_AS: \K\d+' "$TEST_SCRIPT" 2>/dev/null || true)
  if [[ -n "$RLIMIT_AS" ]]; then
    # Convert bytes to kbytes for ulimit -v
    ULIMIT_PREFIX="ulimit -v $((RLIMIT_AS / 1024));"
  fi

  TEST_OUTPUT=$(bash -c "${ULIMIT_PREFIX} timeout $TIMEOUT_DURATION \"$NODE_BIN\" \"$TEST_SCRIPT\"" 2>&1)
  TEST_RESULT=$?
