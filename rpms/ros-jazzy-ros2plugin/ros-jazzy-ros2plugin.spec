Name:           ros-jazzy-ros2plugin
Version:        5.4.6
Release:        1%{?dist}
Summary:        The plugin command for ROS 2 CLI

License:        Apache-2.0
URL:            https://github.com/ros/pluginlib
Source0:        https://github.com/ros/pluginlib/archive/refs/tags/5.4.6.tar.gz#/ros-jazzy-ros2plugin-5.4.6.tar.gz
...
%description
The plugin command for ROS 2 CLI. Part of the ROS 2 Jazzy stack, installed under %{_libdir}/ros-jazzy.

%prep
%autosetup -n pluginlib-5.4.6
# Some monorepos ship LICENSE per-subdir; stage it for %%license.
[ -f LICENSE ] || cp -f ros2plugin/LICENSE LICENSE 2>/dev/null || :
...
colcon build \
    ...
    -DBUILD_TESTING=OFF \
    --base-paths ros2plugin \
    --install-base %{buildroot}%{_libdir}/ros-jazzy/ \
    --packages-select ros2plugin
