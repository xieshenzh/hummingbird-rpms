...
# Fetched by metadata/composer.source-pipeline.yaml: pristine upstream
# source (no vendored dependencies -- see Source2).
Source0:        %{gh_project}-%{gh_tag}.tar.gz
# Profile scripts
Source1:        %{name}-bash-completion
# Vendored PHP dependencies (`composer install --no-dev --optimize-autoloader`),
# generated by metadata/composer.source-pipeline.yaml's vendor: composer step.
Source2:        %{gh_project}-%{gh_tag}-vendor.tar.gz
Source3:        %{name}.sh
Source4:        %{name}.csh
...
BuildArch:      noarch
# platform pin comes from composer's own committed composer.json
# (config.platform.php); gorget's vendor step installs against it as-is.
