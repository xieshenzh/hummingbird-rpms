...
# With BH naming conventions where name == tomcat10
...
Source2:       KEYS
Source3:       tomcat-%{major_version}.%{minor_version}.conf
Source4:       tomcat-%{major_version}.%{minor_version}.service
Source5:       tomcat-%{major_version}.%{minor_version}-locate-java.sh
Source6:       tomcat-%{major_version}.%{minor_version}-run.sh
Source7:       tomcat-%{major_version}.%{minor_version}-RUNNING.txt
Source8:       tomcat-%{major_version}.%{minor_version}-user-instance-create.sh
Source9:       tomcat-%{major_version}.%{minor_version}-setenv.sh
Source10:       tomcat-%{major_version}.%{minor_version}-user-instance-create.asciidoc

# https://bugzilla.redhat.com/show_bug.cgi?id=435829
Patch0:        tomcat-%{major_version}.%{minor_version}-bootstrap-MANIFEST.MF.patch
Patch1:        tomcat-%{major_version}.%{minor_version}-users-webapp.patch
Patch2:        tomcat-%{major_version}.%{minor_version}-build.patch
...
%{__install} -D -p %{SOURCE3} ${RPM_BUILD_ROOT}%{confdir}/%{basepackagename}.conf
%{__install} -D -p %{SOURCE4} ${RPM_BUILD_ROOT}%{_unitdir}/%{basepackagename}.service
%{__install} -D -p %{SOURCE5} ${RPM_BUILD_ROOT}%{_libexecdir}/%{basepackagename}/%{basepackagename}-locate-java.sh
%{__install} -D -p %{SOURCE6} ${RPM_BUILD_ROOT}%{_libexecdir}/%{basepackagename}/%{basepackagename}-run.sh
