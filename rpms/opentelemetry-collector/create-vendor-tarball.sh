#!/usr/bin/bash
# Generate source archives for opentelemetry-collector (otelcol distribution).
#
# Source: opentelemetry-collector-releases repo, not opentelemetry-collector.
# OCB generates _build/{main.go,components.go,go.mod,go.sum} from
# distributions/otelcol/manifest.yaml. go_vendor_archive then vendors from
# _build/ (passed directly, so tarball has bare vendor/ prefix).
#
...
echo "Running OCB with otelcol manifest..." >&2
(cd "${srcdir}" && "${PWD}/ocb" --skip-compilation \
    --config distributions/otelcol/manifest.yaml) >&2
