...
%nodejs_define_version node 1:20.20.3-1.1%{?dist} -p
...
Name:           nodejs%{node_version_major}-lts
Epoch:          %{node_epoch}
...
0005-Fix-configure-version-unpacking-for-Apple-detection.patch
...
Requires:       %{name}-devel%{?_isa} = %{node_evr}
Requires:       %{name}-libs%{?_isa}  = %{node_evr}
...
Group:          Development/Languages
BuildArch:      noarch
Requires:       %{name}-npm = %{npm_evr}
Requires(meta): %{name}-bin = %{node_evr}
