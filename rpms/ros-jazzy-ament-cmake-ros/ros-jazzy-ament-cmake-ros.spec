...
Version:        0.12.2
Release:        1%{?dist}
Summary:        The ROS-specific package for ament_cmake

License:        Apache-2.0
URL:            https://github.com/ros2/ament_cmake_ros
# ros2/ament_cmake_ros ships ament_cmake_ros, ament_cmake_ros_core and
# domain_coordinator in one repo; this spec carves out ament_cmake_ros
# (build_type: ament_cmake) with colcon's --packages-select.
...
# depend: domain_coordinator (build + exec).
# buildtool_export_depend -> Requires: the installed ament_cmake_ros-extras.cmake
# runs find_package(ament_cmake_gmock/gtest/pytest QUIET REQUIRED) UNCONDITIONALLY
# (not under if(BUILD_TESTING)), so every consumer needs all three at configure.
...
%description
The ROS-specific package for the ament buildsystem in CMake: shared-library
defaults, ROS_PACKAGE_NAME definitions, and domain-isolated gtest/gmock/pytest
test helpers. Part of ROS 2 Jazzy, installed under %{_libdir}/ros-jazzy.
