...
# Was `sed -e 's/\([[:digit:]]*\)\.\([[:digit:]]*\)\.[[:digit:]]*$/\1.\2/'`, but
# RPM's %global storage of a %(shell) body mishandles sed backreferences
# (\1/\2) regardless of ordering relative to Version: -- confirmed broken even
# with %global placed after Version: (see HUM-6191). cut has no backreferences
# and needs no eval; verified against both 2- and 3-component version strings.
...
Patch3: 0003-libmount-add-mnt_open_tree-helper-for-safe-tree-ope.patch
Patch4: 0004-libmount-fix-X-mount.subdir-symlink-following-on-de.patch
Patch5: 0005-libmount-restrict-source-path-canonicalization-for-.patch
Patch6: 0006-libmount-pin-source-path-with-openat2-for-restricte.patch
Patch7: 0007-libmount-skip-post-mount-hooks-after-failed-mount-h.patch
