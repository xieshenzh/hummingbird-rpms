...
Version:        3.1.1
Release:        1%{?dist}
Summary:        Common interface definitions for the ROS 2 logging implementations

License:        Apache-2.0
URL:            https://github.com/ros2/rcl_logging
# ros2/rcl_logging ships rcl_logging_interface, rcl_logging_noop and
# rcl_logging_spdlog in one repo; this spec carves out rcl_logging_interface.
Source0:        https://github.com/ros2/rcl_logging/archive/refs/tags/%{version}.tar.gz#/%{name}-%{version}.tar.gz
...
# buildtool_depend: ament_cmake_ros
...
# depend / ament_export_dependencies(rcutils); rcutils' .so is under the ROS
# prefix (not a system ld path), so declare it explicitly.
Requires:       ros-jazzy-rcutils

%description
rcl_logging_interface defines the common C interface that the ROS 2 logging
backend implementations (noop, spdlog) implement, plus helpers to resolve the
logging directory. Installed under %{_libdir}/ros-jazzy.

%prep
%autosetup -n rcl_logging-%{version}
...
colcon build \
    ...
    --base-paths rcl_logging_interface \
