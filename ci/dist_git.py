...
# check_upstream_versions.py lives alongside this script in ci/, but isn't a
# proper installed package -- add this script's own directory to sys.path so
# a plain import finds it, both when run directly (python already does this
# automatically) and when loaded as an in-process module by tests (which
# doesn't).
sys.path.insert(0, str(Path(__file__).resolve().parent))
import check_upstream_versions as cuv
...
class PackageMetadata(TypedDict):
    ...
    source_availability_check: NotRequired[str]  # Named source checker (e.g., "openjdk_osci")
    version_from_ref: NotRequired[dict[str, str]]  # How to derive a fixed-ref pipeline's gorget --version (e.g. gcc)
...
# --- gorget re-invocation after a dist-git merge (HUM-4621) ----------------
#
# A package with metadata/<package>.source-pipeline.yaml is supposed to have
# its `sources` file wholesale-generated by gorget, never hand-merged. But
# update()'s Fedora-branch-pull path merges Fedora's own `sources` changes
# like any other file -- nothing here previously invoked gorget at all. This
# silently shipped a Fedora-sourced, gorget-unverified `sources` checksum
# whose tarball was never uploaded to Hummingbird's own lookaside cache (see
# the !4170 gcc incident that prompted this).
#
# Two triggers, since a package version bump isn't a reliable signal for
# every pipeline shape:
#   - Version-templated pipelines (the common case): old_version != new_version.
#   - Fixed-ref pipelines (gcc, glibc, libyuv, php-patchwork-jsqueeze, vim --
#     git-snapshot pins with no tagged upstream release): the pipeline's own
#     ref: is the source of truth instead, kept in sync with the spec by
#     test_git_fetch_fixed_ref_matches_spec's same logic, reused here.

# Git's own default abbreviated-SHA length.
_FIXED_REF_SHORT_SHA_LEN = 7

_SPEC_CHANGELOG_RE = re.compile(r"^%changelog\b", re.MULTILINE)

# Captures the argument of a %setup/%autosetup -n override, tolerating
# combined short flags (e.g. `-qn`) and other flags before/after it. The
# negative lookbehind anchors "-[a-z]*n" to an actual flag boundary, so a
# hypothetical flag whose body merely ends in "n" can't be misread as -n.
_SPEC_SETUP_N_RE = re.compile(r"%(?:auto)?setup\b[^%\n]*?(?<![a-zA-Z])-[a-z]*n\s+(\S+)")

# Spellings of the plain default directory name that %setup/%autosetup
# already assumes with no -n override at all.
_SPEC_DEFAULT_N_VALUES = {"%{name}-%{version}", "%{name}%{version}"}


def _fixed_pipeline_git_refs(pipeline: dict) -> list[str]:
    """Return `ref:` values of `fetch: type: git` steps that are NOT
    version-templated -- i.e. fixed pins independent of version bumps.

    See test/test_source_pipeline_git_archive_naming.py for the full
    rationale (kept in sync with this logic -- both check the same thing,
    one at CI time, one at update-commit time)."""
    return [
        step["ref"]
...
def _spec_has_custom_setup_override(spec_text: str) -> bool:
    """True if any %setup/%autosetup has an explicit -n argument that isn't
    just the plain default directory name."""
    return any(
        match.group(1) not in _SPEC_DEFAULT_N_VALUES
        for match in _SPEC_SETUP_N_RE.finditer(spec_text)
    )


def _spec_text_before_changelog(spec_text: str) -> str:
    """Strip %changelog and everything after it -- an old commit's short SHA
    can persist there forever as history, masking a stale ref that's no
    longer referenced anywhere in the package's *current* spec content."""
    match = _SPEC_CHANGELOG_RE.search(spec_text)
    return spec_text[:match.start()] if match else spec_text


def _fixed_ref_pipeline_drift(package_dir: Path, pipeline: dict) -> list[str]:
    """Return the subset of `pipeline`'s fixed git refs that no longer appear
    (by short SHA) anywhere in the package's current spec, when that spec
    also has a custom %setup/%autosetup -n override. Empty list means no
    drift detected, or the check doesn't apply to this package (no fixed
    refs, or no custom -n override to carry a stale identifier)."""
    fixed_refs = _fixed_pipeline_git_refs(pipeline)
    if not fixed_refs:
        return []
    spec_files = list(package_dir.glob('*.spec'))
    if len(spec_files) != 1:
        return []
    spec_text = _spec_text_before_changelog(spec_files[0].read_text())
    if not _spec_has_custom_setup_override(spec_text):
        return []
    return [ref for ref in fixed_refs if ref[:_FIXED_REF_SHORT_SHA_LEN] not in spec_text]


# Currently the only fixed-ref pin shape dist_git.py can refresh
# automatically: a full 40-char commit SHA in a `%global gitrev` (gcc's
# convention). glibc's %{glibcsrcdir} (a `git describe` string whose trailing
# hash is only a 10-char abbreviation) needs real extra work to resolve back
# to a full SHA -- a local clone containing the object, plus handling
# ambiguous-hash and unreachable-upstream failure modes -- not automated yet,
# see HUM-4621's "Remaining work". Packages that want this type must use
# gcc's exact `%global gitrev <40-char-sha>` spec convention.
_GITREV_RE = re.compile(r"^%global\s+gitrev\s+([0-9a-fA-F]{40})\s*$", re.MULTILINE)
...
def _refresh_fixed_ref_pin(package_name: str, package_dir: Path, pipeline_file: Path,
    ...
    if ref_type != 'commit-date':
        logging.warning("%s: unsupported version_from_ref type %r", package_name, ref_type)
        return None
    if not upstream_repo:
        logging.warning("%s: version_from_ref requires upstream_repo to be set", package_name)
        return None
    ...
    match = _GITREV_RE.search(spec_text)
    if not match:
        logging.warning("%s: could not find %%global gitrev in merged spec", package_name)
        return None
    new_gitrev = match.group(1)

    old_gitrev = None
    for step in pipeline.get('fetch') or []:
        if (isinstance(step, dict) and step.get('type') == 'git'
                and step.get('repo') == '${UPSTREAM_REPO}'):
            old_gitrev = step.get('ref')
            break
    if old_gitrev is None:
        logging.warning("%s: could not find its own fetch step in the pipeline", package_name)
        return None
    if old_gitrev == new_gitrev:
        return None  # nothing to refresh

    try:
        with tempfile.TemporaryDirectory(prefix=f"{package_name}-pin-refresh-") as tmp:
            subprocess.run(['git', 'init', '--quiet'], cwd=tmp, check=True,
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            subprocess.run(
                ['git', 'fetch', '--quiet', '--depth', '1', upstream_repo, new_gitrev],
                cwd=tmp, check=True, timeout=120,
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
            date_result = subprocess.run(
                ['git', 'log', '-1', '--format=%cd', '--date=format:%Y%m%d', 'FETCH_HEAD'],
                cwd=tmp, check=True, capture_output=True, text=True, timeout=30,
            )
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired, OSError) as exc:
        # OSError also covers TemporaryDirectory creation itself failing (e.g. /tmp
        # full) -- everything in this function must degrade to "needs manual
        # resolution" rather than crashing update() outright.
        logging.warning("%s: could not determine commit date for %s: %s",
                         package_name, new_gitrev, exc)
        return None

    commit_date = date_result.stdout.strip()
    if not re.fullmatch(r'\d{8}', commit_date):
        logging.warning("%s: unexpected commit date output %r for %s",
                         package_name, commit_date, new_gitrev)
        return None

    # Targeted text substitution, not a yaml.safe_load/dump round-trip --
    # this pipeline's extensive prose comments must survive untouched.
    # Quote-agnostic (the ref may be quoted with '"', "'", or not at all)
    # so this doesn't silently stop working if the file's style ever changes.
    pipeline_text = pipeline_file.read_text()
    ref_pattern = re.compile(
        r'(ref:\s*)(["\']?)' + re.escape(old_gitrev) + r'\2'
    )
    new_pipeline_text, count = ref_pattern.subn(
        lambda m: f'{m.group(1)}"{new_gitrev}"', pipeline_text, count=1,
    )
    if count == 0:
...
def _run_gorget_for_updated_package(package_name: str, package_dir: Path,
    ...
    if drifted:
        ...
        if version_from_ref:
            ...
            try:
                uploaded = cuv._run_gorget_pipeline(package_name, old_version, gorget_version, pipeline_file)
...
def update(package_name: str, skip_build_check: bool = False, sync: bool = False,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # plain git merge above produced for Fedora's own `sources` changes
        # (see _run_gorget_for_updated_package). Skipped on an already-broken
        # git-level merge (nothing reliable to verify) and in --dry-run mode
        ...
        if not has_conflicts and not dry_run:
            ...
            if gorget_failure_reason:
                logging.warning("%s", gorget_failure_reason)
                has_conflicts = True
        ...
        if not dry_run:
            if has_conflicts:
                ...
                if gorget_failure_reason:
                    # No git-level conflict markers exist in this case (the
                    # merge itself was clean) -- prefix the commit message so
                    # dist_git_update_multi_mr.sh's conflict detection (which
                    # otherwise only greps for literal "<<<<<<< HEAD") still
                    # routes this to a draft, no-test, human-reviewed MR
                    # instead of silently auto-merging it.
                    commit_msg = f"NEEDS MANUAL GORGET RUN: {commit_msg}\n\n{gorget_failure_reason}"
            ...
            _stage_package_for_commit(package_name, package_dir,
                                       (f'metadata/{package_name}.json',), new_source_filenames)
            verb = "Sync" if sync else "Update"
            branch_suffix = f" ({old_branch} -> {effective_branch})" if branch_changed else ""
            commit_msg = f"{verb} {package_name} from {old_version}-{old_release} to {version}-{release}{branch_suffix}\n\nUpstream: {latest_sha}"
            run_git_commit('-m', commit_msg, cwd=ROOT_DIR)
